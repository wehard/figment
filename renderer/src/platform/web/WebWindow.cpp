...
{
    auto *openGlWindow = (WebWindow*)glfwGetWindowUserPointer(window);
    openGlWindow->Resize({
        .Width = width,
        .Height = height,
...
WebWindow::WebWindow(const std::string &title, const uint32_t width, const uint32_t height) : m_Title(title), m_Width(width), m_Height(height)
{
...
void WebWindow::Resize(WindowResizeEventData resizeData)
{
...
bool WebWindow::ShouldClose()
{
    return glfwWindowShouldClose(m_Window);
}

uint32_t WebWindow::GetWidth() const
{
    return m_Width;
}

uint32_t WebWindow::GetHeight() const
{
    return m_Height;
}
...
void WebWindow::SetResizeEventCallback(ResizeEventCallbackFn callback) {
