...
{
    m_Shader =
        std::make_unique<VulkanShader>(context, "res/shader.vert.spv", "res/shader.frag.spv");
...
void Renderer::InitGui(GLFWwindow* window)
{
    ...
    initInfo.DescriptorPool =
        m_Context.createDescriptorPool({{VK_DESCRIPTOR_TYPE_SAMPLER, 1000},
...
{
    m_GuiPass = std::make_unique<VulkanRenderPass>(
        m_Context,
        VulkanRenderPass::RenderPassDescriptor{
...
{
    checkVkResult(vkWaitForFences(m_Context.GetDevice(), 1,
                                  &m_SynchronizationObjects[m_FrameIndex].FenceDraw, VK_TRUE,
                                  std::numeric_limits<uint64_t>::max()));
...
{
    std::array<VkCommandBuffer, 2> submitCommandBuffers = {m_CommandBuffers[m_FrameIndex],
                                                           m_GuiCommandBuffers[m_FrameIndex]};
    ...
    checkVkResult(vkQueueSubmit(m_Context.GetGraphicsQueue(), 1, &submitInfo,
        ...
        [this]()
        {
            for (auto& m_SynchronizationObject: m_SynchronizationObjects)
            {
                ...
                vkDestroySemaphore(m_Context.GetDevice(),
                                   m_SynchronizationObject.SemaphoreImageAvailable, nullptr);
                vkDestroySemaphore(m_Context.GetDevice(),
                                   m_SynchronizationObject.SemaphoreRenderFinished, nullptr);
            }
        });
}
...
void Renderer::CreateCommandBuffers()
{
    ...
    commandBufferAllocateInfo.level =
        VK_COMMAND_BUFFER_LEVEL_PRIMARY; // can only be executed by queue, not like secondary that
                                         // can be executed by primary command buffers
...
void Renderer::CreateGuiCommandBuffers()
{
    ...
    commandBufferAllocateInfo.commandBufferCount =
        static_cast<uint32_t>(m_GuiCommandBuffers.size());
