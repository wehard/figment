    ...
    m_Context(context)
{
    ...
    memoryAllocateInfo.memoryTypeIndex      = m_Context.findMemoryTypeIndex(
        memoryRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

    checkVkResult(
        vkAllocateMemory(m_Context.GetDevice(), &memoryAllocateInfo, nullptr, &m_ImageMemory));

    vkBindImageMemory(m_Context.GetDevice(), m_Image, m_ImageMemory, 0);

    spdlog::info("VulkanTexture created: width = {}, height = {}", descriptor.Width,
                 descriptor.Height);
    ...
    checkVkResult(
        vkCreateImageView(m_Context.GetDevice(), &imageViewCreateInfo, nullptr, &m_ImageView));
...
void VulkanTexture::CopyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width,
...
glm::vec4 VulkanTexture::GetPixel(int x, int y) const
{
    ...
    auto dstBuffer =
        VulkanBuffer(m_Context, VulkanBufferDescriptor{
