...
namespace Figment
{
    VulkanBindGroup::VulkanBindGroup(const VulkanContext &context,  const BindGroupDescriptor &descriptor) : m_Context(context)
    {
        ...
        CheckVkResult(vkAllocateDescriptorSets(m_Context.GetDevice(), &descriptorSetAllocateInfo, &m_DescriptorSet));
        ...
        for (int i = 0; i < descriptor.Bindings.size(); i++)
        {
            ...
            descriptorWrite.descriptorType = descriptor.Bindings[i].DescriptorSetLayoutBinding.descriptorType;
